...
app.initBox = function () {
    var boxWidth = this.star.width * this.star.colNum + this.star.margin * this.star.colNum;
...
app.initStars = function () {
    for (var i = 0; i < this.star.rowNum; i++) {
        this.stars[i] = [];
        for (var k = 0; k < this.star.colNum; k++) {
...
app.initColsArray = function (val) {
    ...
    for (var i = 0; i < this.star.colNum; i++) {
        array[i] = val;
    }
    return array;
};
...
app.click = function (x, y) {
    ...
    this.linkStars = [];
    this.searchStars = [];
    this.bigSearch(searchStars, false); // 搜索 连接的星星
    this.clickAnimate();
};
...
app.touch = function (x, y) {
    ...
    if (this.linkStars.length >= 2)
        this.score += this.linkStars.length * 10 * (1 + (this.linkStars.length - 2) / 10);
    this.initColsLink();
    this.colsNoneLenth = this.initColsArray(0);
    this.leftData = {min: Number(this.star.colNum), y: [], num: 0, count: this.initColsArray(1)};
    this.makeStars(); // 重新生成星星数组
    this.animate();

    var state = false;
    outterLoop :for (var i = 0; i < app.stars.length; i++) {
        for (var j = 0; j < app.stars[0].length; j++) {
            if (app.stars[i][j] < 0) continue;
            this.count = 0;
            this.linkStars = [];
            this.searchStars = [];
            searchStars[0] = {x: i, y: j};
            if (this.bigSearch(searchStars, true)) {
                state = true;
                break outterLoop;
            }
        }
    ...
    if (!state) {
        var target = parseInt(document.getElementById('target').innerHTML);
        var score = parseInt(document.getElementById('score').innerHTML);
        if (target <= score) {
            alert("You win!");
            $('#target').html(target + 200);
            this.initStars();
            this.draw();
            this.initBox();
        }
    ...
    if (this.leftData.min > -1) {
        if (this.leftData.y.length > 1) {
            ...
            for (var j = 0; j <= this.leftData.y.length - 1; j++) {
                ...
                if (j + 1 > this.leftData.y.length - 1) r = this.star.colNum;
            ...
            for (var n_x = 0; n_x <= this.star.rowNum - 1; n_x++) {
                for (var n_y = y + 1; n_y < this.star.colNum; n_y++) {
...
app.delAnimate = function (x, y) {
    var index = x * this.star.colNum + y; // 根据x、y计算对应dom中星星的 id
    ...
    starDiv.animate({left: left + 'px', top: top + 'px', opacity: 0}, 500, function () {
        $(this).hide();
    });
...
app.downAnimate = function (x, y, move_num) {
    ...
    var index = x * this.star.colNum + y;
    var starDiv = this.starBox.find('.id_' + index);
    ...
    starDiv.removeClass('id_' + index);
    var id = parseInt(n_x_ * this.star.colNum) + parseInt(y);
...
app.leftAnimate = function (x, y, move_num) {
    ...
    starDiv.removeClass('id_' + index);
    var id = parseInt(x * this.star.colNum) + parseInt(n_y_);
    starDiv.addClass('id_' + id);
};
// 点击提示连接星星动画
...
/* 递归遍历查找 */
app.bigSearch = function (searchStars, checkMode) {
    ...
    this.bigSearch(this.newSearchStars, checkMode);
    if (checkMode && this.linkStars.length >= 2) return true;
    return false;
};
/* 上下左右 查找 */
app.search = function (x, y, position) {
    if (position == 'top') {
        ...
        var right = y + 1;
        if (right > this.star.colNum - 1) return;
...
app.makeStars = function () {
    ...
    */
    ...
    var ld_count = 0;
    for (var y = 0; y <= this.star.colNum - 1; y++) {
        y = parseInt(y);
        // if(this.star.colNum - (y+1) < this.colsNoneNum) continue;
        // 判断当前列是否全部被设置为-1
        if (this.checkColsNone(y)) {
            if (this.leftData.min > y) {
                this.leftData.min = y;
            }
            if ((y + 1 <= this.star.colNum - 1 && !this.checkColsNone(y + 1)) || (y == this.star.colNum - 1)) {
                this.leftData.y[ld_count] = y;
                ld_count++;
            }
            else if (y + 1 <= this.star.colNum - 1 && this.checkColsNone(y + 1)) {
    ...
    if (this.leftData.min > -1) {
        ...
        for (var y = 0; y < this.star.colNum; y++) {
            if (this.checkNewColsNone(check)) {
                for (var n_x = 0; n_x <= this.star.rowNum - 1; n_x++) {
                    for (var n_y = check + 1; n_y < this.star.colNum; n_y++) {
                        this.newStars[n_x][n_y - 1] = this.newStars[n_x][n_y];
                    }
                    this.newStars[n_x][this.star.colNum - 1] = -1;
                }
            }
            else {
                check += 1;
...
app.draw = function () {
    ...
    for (var x in this.stars) {
        ...
        for (var y in this.stars[x]) {
            ...
            var index = x * this.star.colNum + y;
            starsDiv += '<div class="star id_' + index + '" style="left:' + left + 'px; top:' + top + 'px; width:' + this.star.width + 'px;height:' + this.star.height + 'px;" ondblclick="app.touch(' + x + ',' + y + ');" onmousedown="app.click(' + x + ',' + y + ');" onmouseup="app.mouseup();"><img src="../images/hitbubble/BUBBLE' + star + '.png" width="' + this.star.width + '" height="' + this.star.height + '"/></div>';
        }
    }
